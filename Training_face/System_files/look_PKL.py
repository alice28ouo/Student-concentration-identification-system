...
def plot_descriptor_distribution(known_faces):
    all_descriptors = np.vstack([face['descriptors'] for face in known_faces])
    mean_descriptor = all_descriptors.mean(axis=0)
    std_descriptor = all_descriptors.std(axis=0)
...
def main():
    ...
    accuracies = analyze_face_data(known_faces)
    plot_accuracies(accuracies)
    plot_descriptor_distribution(known_faces)

    print("Analysis complete. Charts have been saved as 'face_recognition_accuracy.png' and 'face_descriptor_distribution.png'.")
